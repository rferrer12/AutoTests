...
class XYStage:
    ...
    def __init__(self, connection_port: str):
        ...
        self.connection = Connection.open_iot("007750a3-7dd6-41fa-9adb-b9f9451c8174")
    ...
    def move_to(self, abs_pos, move_vel):
        if isinstance(abs_pos, float):
            for i in self.axes:
                if abs_pos > self.axes[i]["limits"][1]:
                    pos = self.axes[i]["limits"][1]
                elif abs_pos < self.axes[i]["limits"][0]:
